...
class Config:
    ...
    eval_steps: List[int] = field(default_factory=lambda: [7_000, 30_000, 40_000, 50_000])
    # Steps to save the model
    save_steps: List[int] = field(default_factory=lambda: [7_000, 30_000, 40_000, 50_000])
    # Whether to save ply file (storage size can be large)
    save_ply: bool = False
    # Steps to save the model as ply
    ply_steps: List[int] = field(default_factory=lambda: [7_000, 30_000, 40_000, 50_000])
    ...
    depth_lambda: float = 1e-4
    # Use precomputed depth maps instead of point-based depth supervision
    ...
    enable_depth_smoothness = True
    enable_normal_consistency = True

    save_depth_normal_vis = True
    depth_normal_vis_frequency = 500 
...
class Runner:
    ...
    def train(self):
        ...
        for step in pbar:
            ...
            if cfg.depth_loss:
                if "depth_map" in data and data.get("has_dense_depth", False):
                    # Use dense refined depth map from PriorDepthAnything
                    depth_map_gt = data["depth_map"].to(device).unsqueeze(0)  # [1, H, W]
                    has_dense_depth = True
                    if render_scale > 1:
                        # Downsample dense depth map to match render resolution
                        depth_map_gt = F.interpolate(
                            depth_map_gt.unsqueeze(1),
                            scale_factor=1.0 / render_scale,
                            mode='nearest',
                            align_corners=True
                        ).squeeze(1)
                elif "points" in data and "depths" in data:
                    # Fall back to sparse COLMAP depths
                    points = data["points"].to(device)  # [1, M, 2]
                    depths_gt = data["depths"].to(device)  # [1, M]
            ...
            if cfg.save_depth_normal_vis and cfg.load_normals:
                add_depth_normal_visualization_to_training_loop(
                    renders=renders,
                    pixels=pixels,
                    camera_K=Ks[0],  # Use first camera's intrinsics
                    step=step,
                    cfg=cfg,
                    world_rank=world_rank
                )
            ...
            # print("Info ", info)
            # print("Means2d", info["means2d"])
            # print("Means2d shape ", info["means2d"].shape)
            # print("Normals ", data["normal"])
            ...
            if cfg.load_normals and step >= cfg.start_normal_loss_step:
                ...
                points_in_view = torch.any(info["means2d"] != 0, dim=-1)
                points_in_view_indices = torch.nonzero(points_in_view, as_tuple=False)

                pixel_coords = info["means2d"][0].long()  # [N, 2]
            ...
            if cfg.depth_loss:
                if has_dense_depth and depth_map_gt is not None:
                    # Dense depth supervision from PriorDepthAnything
                    # depths is [B, H, W, 1] from rendering
                    rendered_depth = depths.squeeze(-1)  # [B, H, W]
                    
                    # Mask out invalid depths
                    valid_mask = (depth_map_gt > 0) & (rendered_depth > 0)
                    
                    if valid_mask.sum() > 0:
                        # Calculate loss in disparity space for better numerical stability
                        disp_rendered = torch.where(valid_mask, 1.0 / rendered_depth, torch.zeros_like(rendered_depth))
                        disp_gt = torch.where(valid_mask, 1.0 / depth_map_gt, torch.zeros_like(depth_map_gt))

                        # if step % 100 == 0:
                        #     save_disparity_image(depth_map_gt, f"{self.viewer.output_dir}/dense_render_depth/dense_disp_gt_{step}.png")

                        # Use L1 loss on valid disparities
                        depthloss = F.l1_loss(disp_rendered[valid_mask], disp_gt[valid_mask]) * self.scene_scale
                        loss += depthloss * cfg.depth_lambda
                        
                elif points is not None and depths_gt is not None:
                    # Sparse depth supervision from COLMAP
                    # query depths from rendered depth map at sparse points
                    points = torch.stack(
                        [
                    ...
                    # if step % 100 == 0:
                    #     save_disparity_image(
                    #         depths_gt, 
                    #         f"{self.viewer.output_dir}/dense_render__depth_colmap/sparse_disp_gt_{step}.png",
                    #         points=data["points"], # Pass the original point coordinates
                    #         height=height,
                    #         width=width
                    #     )
            ...
            if cfg.depth_normal_loss:
                loss, depth_smooth_loss, normal_consistency_loss = add_simplified_depth_normal_loss(
                    renders=renders,
                    pixels=pixels, 
                    camera_K=Ks[0],  # Use first camera's intrinsics
                    cfg=cfg,
            ...
            if cfg.depth_normal_loss and  depth_smooth_loss is not None:
                desc += f"depth_smooth={depth_smooth_loss.item():.6f}| "
            if cfg.depth_normal_loss and normal_consistency_loss is not None:
                desc += f"normal_consist={normal_consistency_loss.item():.6f}| "
            ...
            if step in [i - 1 for i in cfg.save_steps] and cfg.load_normals and step >= cfg.start_normal_loss_step or step == max_steps - 1 and cfg.load_normals and step >= cfg.start_normal_loss_step:
                folder = f"{self.stats_dir}"
                file_path = os.path.join(folder, f"normals_step{step:04d}_rank{self.world_rank}.npz")
                os.makedirs(folder, exist_ok=True)

                normals_numpy = gaussian_normals.detach().cpu().numpy()
                np.save(file_path, normals_numpy)
                print(f"Saved normals to {file_path}")

                save_path=f"{self.render_dir}/normals_step{step:04d}_rank{self.world_rank}.png"
                os.makedirs(os.path.dirname(save_path), exist_ok=True)

                gaussian_normals_save_path = f"{self.render_dir}/gaussian_normals_step{step:04d}_rank{self.world_rank}.png"
                os.makedirs(os.path.dirname(gaussian_normals_save_path), exist_ok=True)

                normal_vis = render_normals_simple(sampled_normals, means2d=info["means2d"][:, valid_mask], image_height=height, image_width=width)
                save_image_tensor(normal_vis, save_path)

                # gaussian_normal_vis = generate_image_from_normals_batch(gaussian_normals, means2d=info["means2d"], image_height=height, image_width=width, rendering_mode='normals')
                gaussian_normal_vis = render_normals_simple(gaussian_normals, means2d=info["means2d"][:, valid_mask], image_height=height, image_width=width)
                save_image_tensor(gaussian_normal_vis, gaussian_normals_save_path)

                gaussian_normals_filled = render_normals_with_interpolation(gaussian_normals, info["means2d"][:, valid_mask], height, width)
                save_image_tensor(gaussian_normals_filled, f"{self.render_dir}/gaussian_normals_filled_step{step:04d}_rank{self.world_rank}.png")
